...
elQueTieneMasEjercitos(Jugador, Pais):- ocupa(Pais, Jugador, CantEjercitos), 
    forall(ocupa(_, _, OtraCantEjer), CantEjercitos >= OtraCantEjer).
...
cumpleObjetivo(ocuparPaises(Cant, Continente), Jugador):- cantidadDePaises(Jugador, Continente, CantPaises), 
    CantPaises >= Cant.
cumpleObjetivo(destruirJugador(JugadorADestruir), _):- not(ocupa(_, JugadorADestruir, _)).

cantidadDePaises(Jugador, Continente, Cant):- jugador(Jugador), 
findall(Pais, estaOcupado(Pais, Continente, Jugador), ListaPaises), length(ListaPaises, Cant).
